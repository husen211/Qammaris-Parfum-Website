@php($activeValue = (string) old('is_active', isset($category) ? (int) $category->is_active : 1))

@if($errors->any())
    <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">
        <p class="font-semibold">Periksa kembali data kategori.</p>
...
<div>
    <label for="name" class="block text-sm font-semibold text-gray-800">Nama kategori</label>
    <input id="name" name="name" type="text" required maxlength="255" value="{{ old('name', $category->name ?? '') }}"
        ...
        placeholder="Contoh: Eau de Parfum (EDP)">
</div>
...
<div class="flex flex-col-reverse gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:justify-end">
    <a href="{{ route('admin.categories.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">Batal</a>
